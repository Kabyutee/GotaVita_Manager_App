...
(function () {
  ...
  function summarizeRows(rows, run) {
    ...
    return {
      ...
      containersReturned: returned,
      delivered,
      pendingDelivery: Math.max(rows.length - delivered, 0)
    };
  }
  ...
  function daily(reference = new Date()) {
    ...
    return details.reduce((report, run) => {
      ...
      report.orders += run.orders;
      report.gallons += run.gallons;
      report.expectedRevenue += run.expectedRevenue;
      report.paid += run.paid;
      report.receivable += run.receivable;
      report.containersReturned += run.containersReturned;
      report.delivered += run.delivered;
      report.pendingDelivery += run.pendingDelivery;
      return report;
    }, {
      date: new Date(reference).toISOString(), runs: [], orders: 0, gallons: 0,
      expectedRevenue: 0, paid: 0, receivable: 0, containersReturned: 0,
      delivered: 0, pendingDelivery: 0
    });
  }
  ...
  function period(period, reference = new Date()) {
    ...
    const paid = rows.filter(o => o.status === "Paid").reduce((s, o) => s + (Number(o.total) || 0), 0);
    return {
      period, start: bounds.start, end: bounds.end, orders: rows.length,
      gallons: rows.reduce((s, o) => s + (Number(o.gallons) || 0), 0),
      expectedRevenue: revenue, paid, receivable: Math.max(revenue - paid, 0)
    };
  }
