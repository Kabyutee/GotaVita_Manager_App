...
(function () {
  ...
  function stableRowId(row){if(row?.legacy_id!=null&&String(row.legacy_id).trim()!=="")return String(row.legacy_id).trim();if(row?.legacyId!=null&&String(row.legacyId).trim()!=="")return String(row.legacyId).trim();if(row?.legacy_payload?.legacy_id!=null&&String(row.legacy_payload.legacy_id).trim()!=="")return String(row.legacy_payload.legacy_id).trim();if(row?.legacy_payload?.legacyId!=null&&String(row.legacy_payload.legacyId).trim()!=="")return String(row.legacy_payload.legacyId).trim();if(row?.id!=null&&String(row.id).trim()!=="")return String(row.id).trim();return null;}
  ...
  function stateConcurrencyDigest(snapshot){try{return JSON.stringify({orders:Array.isArray(snapshot?.orders)?snapshot.orders:[],deletedOrders:Array.isArray(snapshot?.deletedOrders)?snapshot.deletedOrders:[]});}catch(_){return "";}}
  ...
  async function run(force=false,retryDepth=0){if(!navigator.onLine||window.location.protocol==="file:")return{ok:false,status:"offline-or-local"};if(window.GVData?.isConfigured?.()!==true)return{ok:false,status:"not-configured"};if(!window.GVConflictDetector?.resolveConflictPolicy)return{ok:false,status:"policy-unavailable"};if(sessionStorage.getItem(RUN_LOCK_KEY)==="1")return{ok:false,status:"locked"};sessionStorage.setItem(RUN_LOCK_KEY,"1");try{await window.GVData.requireAuthenticatedManager();const baseline=getBaseline(),nextState=stateSnapshot();if(!nextState)throw new Error("Application state snapshot unavailable.");const initialConcurrencyDigest=stateConcurrencyDigest(nextState),results=[],nextBaseline={...baseline};for(const resource of supportedResources()){const stateName=resourceStateName(resource),localRows=Array.isArray(nextState[stateName])?nextState[stateName]:[],remoteRows=await window.GVData.selectResource(resourceCloudName(resource)),baselineAt=baseline[resource]?.baselineAt||null,baselineRows=Array.isArray(baseline[resource]?.rows)?baseline[resource].rows:[],localDeletedRows=resource==="orders"?(nextState.deletedOrders||[]):[],remoteDeletedRows=resource==="orders"?await window.GVData.selectResource("deleted_orders"):[];const result=await reconcileResource(resource,localRows,remoteRows,baselineAt,localDeletedRows,remoteDeletedRows,baselineRows,nextState);results.push({...result,status:baselineAt?undefined:"baseline-initialized"});const refreshed=await window.GVData.selectResource(resourceCloudName);nextBaseline[resource]={baselineAt:new Date().toISOString(),rows:clone(refreshed)};if(!result.partial)removeResourceFromQueue(resourceCloudName);}if(typeof window.GVGroupMembershipBridge?.reconcileRemoteState==="function")window.GVGroupMembershipBridge.reconcileRemoteState(nextState);const currentState=stateSnapshot();if(stateConcurrencyDigest(currentState)!==initialConcurrencyDigest){if(retryDepth<1){console.warn("GotaVita conflict integration: concurrent local mutation detected; retrying from newest state.");sessionStorage.removeItem(RUN_LOCK_KEY);return await run(force,retryDepth+1);}console.warn("GotaVita conflict integration: concurrent local mutation persisted; preserving newest local state.");return{ok:false,status:"concurrent-local-mutation",results};}if(typeof window.replaceState==="function")window.replaceState(nextState);if(typeof window.persistState==="function")window.persistState();setBaseline(nextBaseline);const manualReviewCount=results.reduce((sum,r)=>sum+(r.summary?.manualReview||0),0),appliedCount=results.reduce((sum,r)=>sum+(r.summary?.keepLocal||0)+(r.summary?.keepRemote||0),0);if(typeof window.setSyncStatus==="function")window.setSyncStatus(manualReviewCount?`Conflict review required · ${manualReviewCount}`:`Synced · ${appliedCount} reconciliation decision(s) applied`,manualReviewCount?"warning":"online");return{ok:true,status:manualReviewCount?"manual-review":"reconciled",results};}finally{sessionStorage.removeItem(RUN_LOCK_KEY);}}
