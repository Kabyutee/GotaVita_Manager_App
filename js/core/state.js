/* GotaVita Manager — state factory and post-auth hydration. */
(function () {
  "use strict";
  ...
  async function hydrateAuthorizedStateAfterAuth() {
    try {
      ...
      const resources = [
        ...
        ["employees", "employees"], ["orders", "orders"], ["payments", "payments"],
        ["expenses", "expenses"], ["payroll_records", "payrollRecords"],
        ["order_groups", "orderGroups"], ["delivery_routes", "deliveryRoutes"],
        ["order_group_items", "orderGroupItems"], ["delivery_route_items", "deliveryRouteItems"],
        ["daily_reports", "dailyReports"], ["deleted_orders", "deletedOrders"]
      ];
  ...
  function scheduleAuthorizedHydration() {
    ...
    const run = () => hydrateAuthorizedStateAfterAuth().finally(() => {
      window.__GV_AUTH_HYDRATION_PROMISE = null;
    });
  ...
  if (typeof document !== "undefined" && typeof document.addEventListener === "function") {
    document.addEventListener("submit", (event) => {
      if (event?.target?.id === "orderForm") reconcileOrderCounterBeforeCreate();
    }, { capture: true });
    ...
    return new Promise((resolve, reject) => {
      ...
      if (existing) {
        existing.addEventListener("load", () => {
          try { window.GVApplicationLifecycleGuard?.install?.(); resolve(); }
