/* GotaVita Manager — single canonical synchronization coordinator. */
(function () {
  "use strict";
  ...
  function appReady() {
    return window.__GV_APP_READY === true || (authorized() && window.GVPerformance && Number.isFinite(Number(window.GVPerformance.lastRenderMs)));
  }
  ...
  function ensureEmergencyRecoveryLoaded() {
    if (typeof document === "undefined" || window.GVEmergencyRecovery) return;
  ...
  async function hydrateFirstBaseline(integration) {
    ...
    let changed = false;
    for (const resource of window.GVData.supportedResources()) {
  ...
  function attachLifecycle() {
    ...
    document.addEventListener("visibilitychange", () => {
      if (document.visibilityState === "visible" && (appReady() || authorized())) flush().catch(() => {});
    });
