...
(function () {
  ...
  function queue() { try { if (typeof window.getSyncQueue === "function") return window.getSyncQueue(); } catch (_) {} const legacy = readJson(QUEUE_KEY, []); return Array.isArray(legacy) ? legacy : []; }
  function clearQueue() { try { if (typeof window.setSyncQueue === "function") window.setSyncQueue([]); } catch (_) {} try { localStorage.removeItem(QUEUE_KEY); } catch (_) {} }
  function createQueueId() { if (typeof crypto?.randomUUID === "function") return crypto.randomUUID(); if (typeof crypto?.getRandomValues === "function") { const bytes = new Uint8Array(16); crypto.getRandomValues(bytes); return [...bytes].map((value) => value.toString(16).padStart(2, "0")).join(""); } return `${Date.now()}-${performance.now().toString(36)}`; }
  function enqueue(payload, entity = "state", action = "upsert") { const resources = Array.isArray(payload) ? payload.filter(Boolean) : [entity].filter(Boolean); try { if (typeof window.queueSyncResources === "function") { window.queueSyncResources(resources.length ? resources : ["state"]); return resources[0] || "state"; } } catch (_) {} const current = queue(); current.push({ id: createQueueId(), entity, action, payload, createdAt: new Date().toISOString(), attempts: 0 }); writeJson(QUEUE_KEY, current); return entity; }
  ...
  function appReady() { return window.__GV_APP_READY === true || (authorized() && window.GVPerformance && Number.isFinite(Number(window.GVPerformance.lastRenderMs))); }
  function activeEditableControl() { try { const active = document.activeElement; return active?.closest?.("input:not([type='checkbox']), select, textarea, button") || null; } catch (_) { return null; } }
  function interactionProtected() { return Boolean(activeInteraction || activeEditableControl()); }
  function beginInteraction() { activeInteraction = true; if (releaseTimer) clearTimeout(releaseTimer); releaseTimer = null; }
  function endInteractionSoon() { if (releaseTimer) clearTimeout(releaseTimer); releaseTimer = setTimeout(() => { releaseTimer = null; if (activeEditableControl()) return; activeInteraction = false; if (deferredRender) { deferredRender = false; renderRemoteState(); } }, INTERACTION_RELEASE_MS); }
  function checkboxKey(control, index) { return [control.dataset?.orderId, control.dataset?.id, control.id, control.name, control.value, index].find((value) => value != null && String(value) !== "")?.toString() || `index:${index}`; }
  function captureBulkSelections() { if (typeof document === "undefined") return []; return [...document.querySelectorAll(".order-checkbox, .billing-checkbox, .all-order-checkbox")].filter((control) => control.type === "checkbox" && control.checked).map((control, index) => ({ className: control.className, key: checkboxKey(control, index) })); }
  function restoreBulkSelections(snapshot) { if (!snapshot?.length || typeof document === "undefined") return; const wanted = new Set(snapshot.map((item) => `${item.className}::${item.key}`)); [...document.querySelectorAll(".order-checkbox, .billing-checkbox, .all-order-checkbox")].filter((control) => control.type === "checkbox").forEach((control, index) => { if (wanted.has(`${control.className}::${checkboxKey(control, index)}`)) control.checked = true; }); }
  ...
  async function hydrateFirstBaseline(integration) { if (!window.GVData?.supportedResources || typeof window.GVData.selectResource !== "function") return false; if (typeof window.getStateSnapshot !== "function" || typeof window.replaceState !== "function") return false; const baseline = integration?.getBaseline?.() || {}; const state = window.getStateSnapshot(); let changed = false; const supported = window.GVData.supportedResources(); for (const resource of supported) { if (resource === "audit_logs") continue; const stateName = integration.resourceStateName ? integration.resourceStateName(resource) : resource; if (!stateName || baseline[resource]) continue; const localRows = Array.isArray(state[stateName]) ? state[stateName] : []; const remoteRows = await window.GVData.selectResource(resource); if (!localRows.length && remoteRows.length) { state[stateName] = remoteRows; changed = true; } } if (!changed) return false; const now = Date.now(); state._meta = Object.assign({}, state._meta, { lastUpdated: now, lastSynchronizedAt: now }); window.replaceState(state); if (typeof window.writeLocalStateSnapshot === "function") window.writeLocalStateSnapshot(state); return true; }
  async function flush() {
    if (window.__GV_ORDER_DIRECT_WRITE_ACTIVE === true) return { ok: false, status: "order-write-in-flight", queued: queue().length };
    ...
    try {
      ...
      try { if (typeof window.GVOrderDeleteReconciliation?.apply === "function") deletionApplied = await window.GVOrderDeleteReconciliation.apply(); } catch (error) { console.warn("GotaVita order tombstone apply:", error?.message || error); }
      let canonicalResourcesApplied = false;
      try { if (typeof window.GVRemoteCanonicalState?.reconcile === "function") canonicalResourcesApplied = await window.GVRemoteCanonicalState.reconcile(); } catch (error) { console.warn("GotaVita canonical resource finalization:", error?.message || error); }
      ...
      if (result?.ok === true) {
        ...
        const meta = getMeta();
        setMeta({ ...meta, lastSyncAt: new Date().toISOString(), lastSyncStatus: manualReview ? "conflict" : (result.status || "synced"), lastSyncQueuedBefore: queuedBefore, lastSyncStateChanged: changed, lastSyncResults: result.results || [], lastDeletionApplied: deletionApplied, lastCanonicalResourcesApplied: canonicalResourcesApplied });
    ...
    } finally { window.__GV_SYNC_TRANSACTION_ACTIVE = false; inFlight = false; }
  }
  async function poll() { return flush(); }
  function startPolling() { if (!authorized() && window.__GV_APP_READY !== true) return; if (timer) return; timer = setInterval(() => { flush().catch(() => {}); }, POLL_MS); flush().catch(() => {}); }
  ...
  window.addEventListener("DOMContentLoaded", () => { ensureEmergencyRecoveryLoaded(); if (typeof window.stopSyncReliability === "function") window.stopSyncReliability(); window.syncChangedResources = () => window.GVSync.flush(); window.syncNow = () => window.GVSync.flush(); window.startSyncReliability = () => {}; startPolling(); }, { once: true });
  attachLifecycle();
  if (window.__GV_APP_READY === true) { ensureEmergencyRecoveryLoaded(); startPolling(); }
})();
