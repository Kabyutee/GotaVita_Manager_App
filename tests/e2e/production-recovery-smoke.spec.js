...
async function login(page, name = 'browser') {
  ...
  page.on('response', response => {
    ...
    if (url.includes('/auth/v1/')) authResponses.push(`${response.status()} ${response.request().method()} ${url}`);
  });
...
test('production Browser A/B order create-edit-delete convergence', async ({ browser }) => {
  ...
  try {
    ...
    await expect.poll(() => pageA.locator('#clientSelect option').count(), { timeout: 30000, intervals: [500, 1000, 2000] }).toBeGreaterThan(1);
    await expect.poll(() => pageA.locator('#custTypeSelect option').count(), { timeout: 30000, intervals: [500, 1000, 2000] }).toBeGreaterThan(0);
