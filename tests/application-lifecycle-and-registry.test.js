...
const state = { clients: [], orders: [], auditLog: [], _meta: {} };
...
const windowObj = {
  ...
  GVSync: { stopPolling: () => { pollingStopped++; }, flush: async () => { syncCalled++; return { ok: true }; } },
  GVAuth: { requireManagerSession: async () => ({ profile: { company_id: "company-1" } }) },
};
windowObj.window = windowObj;
windowObj.location = { protocol: "https:" };
windowObj.navigator = { onLine: true };
...
const context = vm.createContext({
  ...
  document: { querySelector: () => null, head: { appendChild() {} }, documentElement: {} },
  console,
  Date,
  Promise,
  CustomEvent,
...
assert.equal(originalHealthCalled, 0, "health boundary must not invoke the mutating gateway health wrapper");
